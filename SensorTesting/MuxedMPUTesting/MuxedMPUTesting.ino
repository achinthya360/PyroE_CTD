...
void loop() {
  for (uint8_t t = 0; t < 1; t++) {
    ...
    curTime = millis();
    curAcc = IMU.getAccelZ_mss();
    curVel = pastVel + ((curAcc + pastAcc)/2 * (curTime - pastTime)/1000);
    curPos = pastPos + ((curVel + pastVel)/2 * (curTime - pastTime)/1000);
